...
{
	/// <summary>
	/// 数据访问类Approval_Mode_NoteDAL。
	/// </summary>
	public partial class Approval_Mode_NoteDAL
	{
		...
		#region 构造方法
		public Approval_Mode_NoteDAL(DBHelper DB)
		{
				this.FactoryID =  DB.FactoryID;
				this.dbHelper = DB;
		}
		public Approval_Mode_NoteDAL(int factoryID)
		{
		...
		public int GetMaxId()
		{
		return dbHelper.GetMaxID("RKEY", "Approval_Mode_Note"); 
		}
		...
		public bool Exists(int RKEY)
		{
			...
			strSql.Append("select count(1) from Approval_Mode_Note with(nolock) ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Approval_Mode_Note set ");
		...
		public void Delete(int RKEY)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Approval_Mode_Note ");
			strSql.Append(" where RKEY=@RKEY ");
			SqlParameter[] parameters = {
					new SqlParameter("@RKEY", SqlDbType.Int,4)};
			parameters[0].Value = RKEY;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select RKEY,FILE_POINTER,SOURCE_TYPE,NOTES ");
			strSql.Append(" FROM Approval_Mode_Note with(nolock)  ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" RKEY,FILE_POINTER,SOURCE_TYPE,NOTES ");
			strSql.Append(" FROM Approval_Mode_Note with(nolock) ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "Approval_Mode_Note";
