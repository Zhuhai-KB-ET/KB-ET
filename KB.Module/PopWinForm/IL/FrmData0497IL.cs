...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0497IL
    {
        #region 实例化
        DL.FrmData0497DL FrmData0497DL = null;
        FrmData0497 Frm;
        public FrmData0497IL(FrmData0497 psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0497_Load(object sender, EventArgs e)
        {
            try
            {
                FrmData0497DL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0497DL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                FrmData0497DL.BinData(" APPROVAL_ROUTE_DESC like '%" + Frm.textBoxFind.Text.Trim() + "%' or APPROVAL_ROUTE_CODE like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    FrmData0497DL.BinData("APPROVAL_ROUTE_CODE like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
