...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0005IL
    {
        #region 实例化
        DL.FrmData0005DL FrmData0005DL = null;
        FrmData0005 Frm;
        public FrmData0005IL(FrmData0005 psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0005_Load(object sender, EventArgs e)
        {
            try
            {
                FrmData0005DL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0005DL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                FrmData0005DL.BinData(" EMPLOYEE_NAME like '%" + Frm.textBoxFind.Text.Trim() + "%' or EMPL_CODE like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    FrmData0005DL.BinData("EMPL_CODE like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
