...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0050SalesIL
    {
        #region 实例化
        DL.FrmData0050SalesDL frmData0050SalesDL = null;
        FrmData0050Sales Frm;
        public FrmData0050SalesIL(FrmData0050Sales psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0050Sales_Load(object sender, EventArgs e)
        {
            try
            {
                frmData0050SalesDL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0050SalesDL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                frmData0050SalesDL.BinData(" CUSTOMER_PART_DESC like '%" + Frm.textBoxFind.Text.Trim() + "%' or CUSTOMER_PART_NUMBER like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    frmData0050SalesDL.BinData("CUSTOMER_PART_NUMBER like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
