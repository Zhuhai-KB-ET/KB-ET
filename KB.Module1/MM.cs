...
namespace ModelManageSystem
{
    ...
    {
        #region  公共变量
        DataClass.MyMeans MyDataClass = new TMPT.DataClass.MyMeans();   //声明MyMeans类的一个对象，以调用其方法
        ...
        {
            cobox.Items.Clear();
            DataClass.MyMeans MyDataClsaa = new TMPT.DataClass.MyMeans();
        ...
        #region  向comboBox控件传递姓名
        /// <summary>
        /// 动态向comboBox控件的下拉列表添加姓名.
        /// </summary>
        /// <param name="cobox">comboBox控件</param>
        /// <param name="SQLstr">SQL语句</param>
        /// <param name="n">字段位数</param>
        public void NameInfo(ComboBox cobox, string SQLstr, int n)
        {
            cobox.Items.Clear();
            DataClass.MyMeans MyDataClsaa = new TMPT.DataClass.MyMeans();
        ...
        public void Correlation_Table(DataSet DSet, DataGridView DGrid)
        {
            ...
            DGrid.Columns[2].Width = 130;
            DGrid.Columns[3].Width = 100;
            DGrid.Columns[4].Width = 100;
            DGrid.Columns[5].Width = 100;
            DGrid.Columns[6].Width = 100;
            DGrid.Columns[6].Width = 100;
            DGrid.Columns[7].Width = 100;
            DGrid.Columns[8].Width = 150;
        ...
        public void SaveImage(string MID, byte[] p1, byte[] p2, byte[] p3)
        {
            ...
            strSql1.Append("update tb_商标基本信息 Set 商标图片=@photo where ID=" + MID);
            strSql2.Append("update tb_商标基本信息 Set 人物图片=@photo where ID=" + MID);
            strSql3.Append("update tb_商标基本信息 Set 公司图片=@photo where ID=" + MID);
            SqlCommand cmd1 = new SqlCommand(strSql1.ToString(), DataClass.MyMeans.My_con);
            SqlCommand cmd2 = new SqlCommand(strSql2.ToString(), DataClass.MyMeans.My_con);
            SqlCommand cmd3 = new SqlCommand(strSql3.ToString(), DataClass.MyMeans.My_con);

            cmd1.Parameters.Add("@photo", SqlDbType.Binary).Value = p1;
            cmd2.Parameters.Add("@photo", SqlDbType.Binary).Value = p2;
            cmd3.Parameters.Add("@photo", SqlDbType.Binary).Value = p3;
                    ...
                    if (sID.IndexOf(TName) > -1)
                    {
                        if (((CheckBox)C).Checked == true)
                        {
                            ...
                            else
                            {
                                ...
                                if (Astr[1].ToUpper() == ("登录").ToUpper())
                                {
                                    MyDataClass.getsqlcom("insert into " + TableName + " (ID,用户名,密码) values('0001','0902020021013','690906')");
                                    ADD_Pope("0001", 1);
                                }
                            }
                        }
        ...
        {
            DataSet DSet;
            DSet = MyDataClass.getDataSet("select 权限名 from tb_权限模式", "tb_权限模式");
            for (int i = 0; i < DSet.Tables[0].Rows.Count; i++)
            {
                MyDataClass.getsqlcom("insert into tb_用户权限 (ID,权限名,权限) values('" + ID + "','" + Convert.ToString(DSet.Tables[0].Rows[i][0]) + "'," + n + ")");
            }
        }
        #endregion
        ...
        public void Show_Pope(Control.ControlCollection GBox, string TID)
        {
            ...
            string CheckName = "";
            bool t = false;
            DataSet DSet = MyDataClass.getDataSet("select ID,权限名,权限 from tb_用户权限  where ID='" + TID + "'", "tb_用户权限");
            for (int i = 0; i < DSet.Tables[0].Rows.Count; i++)
            {
        ...
        public void Amend_Pope(Control.ControlCollection GBox, string TID)
        {
            ...
            foreach (Control C in GBox)
            {
                if (C.GetType().Name == "CheckBox")
                {
                    ...
                    MyDataClass.getsqlcom("update tb_用户权限 set 权限=" + tt + " where (ID='" + TID + "') and (权限名='" + Astr[1].Trim() + "')");
                }
            }
        ...
        public void MainPope(MenuStrip MenuS, String UName)
        {
            string Str = "";
            string MenuName = "";
            DataSet DSet = MyDataClass.getDataSet("select ID from tb_登录 where 用户名='" + UName + "'", "tb_登录");    //获取当前登录用户的信息
            string UID = Convert.ToString(DSet.Tables[0].Rows[0][0]);   //获取当前用户编号
            DSet = MyDataClass.getDataSet("select  ID,权限名,权限 from tb_用户权限 where ID='" + UID + "'", "tb_用户权限");    //获取当前用户的权限信息
            bool bo = false;
