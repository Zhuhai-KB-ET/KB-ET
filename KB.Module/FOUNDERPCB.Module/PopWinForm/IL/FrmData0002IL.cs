...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0002IL
    {
        #region 实例化
        DL.FrmData0002DL FrmData0002DL = null;
        FrmData0002 Frm;
        public FrmData0002IL(FrmData0002 psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0002_Load(object sender, EventArgs e)
        {
            try
            {
                FrmData0002DL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0002DL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                FrmData0002DL.BinData(" UNIT_NAME like '%" + Frm.textBoxFind.Text.Trim() + "%' or UNIT_CODE like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    FrmData0002DL.BinData("UNIT_CODE like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
