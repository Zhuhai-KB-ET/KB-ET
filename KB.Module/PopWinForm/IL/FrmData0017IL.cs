...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0017IL
    {
        #region 实例化
        DL.FrmData0017DL FrmData0017DL = null;
        FrmData0017 Frm;
        public FrmData0017IL(FrmData0017 psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0017_Load(object sender, EventArgs e)
        {
            try
            {
                FrmData0017DL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0017DL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                FrmData0017DL.BinData(" INV_PART_NUMBER like '%" + Frm.textBoxFind.Text.Trim() + "%' or INV_PART_DESCRIPTION like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    FrmData0017DL.BinData("INV_PART_NUMBER like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
