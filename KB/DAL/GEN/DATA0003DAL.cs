...
namespace KB.DAL
{
	...
	/// 数据访问层   DATA0003DAL
	/// </summary>
	public  partial class DATA0003DAL
	{ 
		#region   字段 and 属性
		...
		/// 构造函数
		/// </summary> 
		public 	DATA0003DAL(Form frm)
		{ 
		    this.FactoryID = GlobalVal.UserInfo.FactoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(frm);
		}
		public 	DATA0003DAL(Form frm, int factoryID)
		{ 
		    this.FactoryID = factoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(frm);
		}
		public 	DATA0003DAL(int Thread, int factoryID)
		{ 
		    this.FactoryID = factoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(Thread, this.FactoryID);
		}
		public 	DATA0003DAL(int Thread)
		{ 
		    this.FactoryID = GlobalVal.UserInfo.FactoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(Thread, this.FactoryID);
		}
		public	DATA0003DAL(DBHelper DB)
        {
		...
		{	
            ...
            strSql.Append("insert into DATA0003(");
			strSql.Append("TDATE,FLAG");
			...
			new SqlParameter("@FLAG",SqlDbType.VarChar,1)
					...
					if (data0003.TDATE == DateTime.Parse("1900-1-1") || data0003.TDATE == DateTime.Parse("0001-1-1"))
						...
						parameters[0].Value=data0003.TDATE;				    
			       parameters[1].Value=data0003.FLAG;
			#endregion
		...
		public   int Update(DATA0003 data0003)
		{
            ...
            try
            {
				dbHelper.ExecuteCommandProc(sql, parameters);
				result = int.Parse(parameters[0].Value.ToString()); 
				log.Info("FID="+this.factoryID.ToString()+";userAD="+this.userAD+";DATA0003,update successful");
            }
            catch (Exception e)
            {
		...
		public   void Update(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, DATA0003 data0003)
		{
			#region 创建语法
			StringBuilder strSql = new StringBuilder();
			strSql.Append("update DATA0003 set "); 
			...
			parameters[0].Value = data0003.RKEY;
					if (data0003.TDATE == DateTime.Parse("1900-1-1") || data0003.TDATE == DateTime.Parse("0001-1-1"))
						parameters[1].Value = null;
					else
						parameters[1].Value=data0003.TDATE;				    
			       parameters[2].Value=data0003.FLAG;
			#endregion
		...
		/// 删除  
		///</sumary>
		/// <param name="data0003">对象</param>
		///<returns>返回INT类型号, 0为操作成功, 非0操作失败.</returns>		
		public   int Delete(DATA0003 data0003)
		{
			#region 调用SQL存储过程进行删除
			string sql="sp_DATA0003_Delete";
			//=========================
			SqlParameter[] parameters={
			new SqlParameter("@returnID",SqlDbType.Int),
			new SqlParameter("@userAD",SqlDbType.VarChar),
            ...
            { 
				dbHelper.ExecuteCommandProc(sql, parameters);
				result = int.Parse(parameters[0].Value.ToString());
				log.Info("FID="+this.factoryID.ToString()+";userAD="+this.userAD+";DATA0003,delete successful");
            }
            catch (Exception e)
            {
				result=2;
		...
		{
			#region 调用SQL存储过程进行删除
			string sql="delete from dbo.DATA0003 where RKEY='"+rkey+"'";
			int result=0;
		...
		public void Delete(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, decimal rkey)
		{
			#region 创建语法
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from data0003 ");
			strSql.Append(" where RKEY=@RKEY ");
		...
		public DATA0003 getDATA0003ByRKEY(decimal rkey)
		{
			...
			from DATA0003 with (nolock) where RKEY='{0}'";
             
			#endregion
			
			///定义返回对象
			DATA0003  data0003=null;
			
			#region 数据库操作
            try
            {
				 data0003=new DATA0003();
				
				using(DataTable tb=dbHelper.GetDataSet(string.Format(sql,rkey)) )
				{
					foreach(DataRow row in tb.Rows)
					{	
							    data0003.RKEY=decimal.Parse(row["RKEY"].ToString()) ;
							  	        data0003.TDATE =DateTime.Parse(row["TDATE"].ToString());
								   data0003.FLAG =row["FLAG"].ToString();
		...
		///<returns>IList<DATA0003>数据集合</returns>		
		public IList< DATA0003> FindBySql(string sqlWhere)
		{
			...
			IList<DATA0003> resultList=new List<DATA0003>();
            ...
            { 
				using(DataTable tb=dbHelper.GetDataSet(sql)) 
				{
					foreach(DataRow row in tb.Rows)
					{
							DATA0003  data0003 =new DATA0003();
