...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0050CustIL
    {
        #region 实例化
        DL.FrmData0050CustDL frmData0050CustDL = null;
        FrmData0050Cust Frm;
        public FrmData0050CustIL(FrmData0050Cust psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0050Cust_Load(object sender, EventArgs e)
        {
            try
            {
                frmData0050CustDL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0050CustDL(Frm);

                Frm.textBoxFind.CharacterCasing = CharacterCasing.Upper;

                Frm.textBoxFind.Text = Frm.is_CustCode.Trim();
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                frmData0050CustDL.BinData(" CUSTOMER_PART_DESC like '%" + Frm.textBoxFind.Text.Trim() + "%' or CUSTOMER_PART_NUMBER like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    frmData0050CustDL.BinData("CUSTOMER_PART_NUMBER like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
