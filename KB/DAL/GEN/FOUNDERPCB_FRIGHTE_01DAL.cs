...
{
	/// <summary>
	/// 数据访问层   FOUNDERPCB_FRIGHTE_01DAL
	/// </summary>
	public  partial class FOUNDERPCB_FRIGHTE_01DAL
	{ 
		#region   字段 and 属性
		...
		/// 构造函数
		/// </summary> 
		public 	FOUNDERPCB_FRIGHTE_01DAL(Form frm)
		{ 
		    this.FactoryID = GlobalVal.UserInfo.FactoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(frm);
		}
		public 	FOUNDERPCB_FRIGHTE_01DAL(Form frm, int factoryID)
		{ 
		    this.FactoryID = factoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(frm);
		}
		public 	FOUNDERPCB_FRIGHTE_01DAL(int Thread, int factoryID)
		{ 
		    this.FactoryID = factoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(Thread, this.FactoryID);
		}
		public 	FOUNDERPCB_FRIGHTE_01DAL(int Thread)
		{ 
		    this.FactoryID = GlobalVal.UserInfo.FactoryID;
			this.UserAD    = GlobalVal.UserInfo.LoginName;
			this.dbHelper  = new DBHelper(Thread, this.FactoryID);
		}
		public	FOUNDERPCB_FRIGHTE_01DAL(DBHelper DB)
        {
		...
		{	
            ...
            strSql.Append("insert into FOUNDERPCB_FRIGHTE_01(");
			strSql.Append("UP_MODULE_ID,MODULE_ID,MODULE_ENGLISH,MODULE_NAME,MODULE_DESC");
			...
			new SqlParameter("@MODULE_DESC",SqlDbType.VarChar,500)
			       ...
			       parameters[0].Value=founderpcb_frighte_01.UP_MODULE_ID;
			       parameters[1].Value=founderpcb_frighte_01.MODULE_ID;
			       parameters[2].Value=founderpcb_frighte_01.MODULE_ENGLISH;
			       parameters[3].Value=founderpcb_frighte_01.MODULE_NAME;
			       parameters[4].Value=founderpcb_frighte_01.MODULE_DESC;
			#endregion
		...
		public   int Update(FOUNDERPCB_FRIGHTE_01 founderpcb_frighte_01)
		{
			...
			string sql="sp_FOUNDERPCB_FRIGHTE_01_Update";
			//=====
            ...
            try
            {
				dbHelper.ExecuteCommandProc(sql, parameters);
				result = int.Parse(parameters[0].Value.ToString()); 
			//	log.Info("FID="+this.factoryID.ToString()+";userAD="+this.userAD+";FOUNDERPCB_FRIGHTE_01,update successful");
            }
            catch (Exception e)
            {
		...
		public   void Update(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, FOUNDERPCB_FRIGHTE_01 founderpcb_frighte_01)
		{
			#region 创建语法
			StringBuilder strSql = new StringBuilder();
			strSql.Append("update FOUNDERPCB_FRIGHTE_01 set "); 
			strSql.Append("UP_MODULE_ID=@UP_MODULE_ID,");
			strSql.Append("MODULE_ID=@MODULE_ID,");
			strSql.Append("MODULE_ENGLISH=@MODULE_ENGLISH,");
			strSql.Append("MODULE_NAME=@MODULE_NAME,");
			strSql.Append("MODULE_DESC=@MODULE_DESC");
			...
			parameters[0].Value = founderpcb_frighte_01.RKEY;
			       parameters[1].Value=founderpcb_frighte_01.UP_MODULE_ID;
			       parameters[2].Value=founderpcb_frighte_01.MODULE_ID;
			       parameters[3].Value=founderpcb_frighte_01.MODULE_ENGLISH;
			       parameters[4].Value=founderpcb_frighte_01.MODULE_NAME;
			       parameters[5].Value=founderpcb_frighte_01.MODULE_DESC;
			#endregion
		...
		public   int Delete(FOUNDERPCB_FRIGHTE_01 founderpcb_frighte_01)
		{
			...
			string sql="sp_FOUNDERPCB_FRIGHTE_01_Delete";
			//=========================
			SqlParameter[] parameters={
			new SqlParameter("@returnID",SqlDbType.Int),
			new SqlParameter("@userAD",SqlDbType.VarChar),
		...
		public void Delete(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, decimal rkey)
		{
			#region 创建语法
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from founderpcb_frighte_01 ");
			strSql.Append(" where RKEY=@RKEY ");
		...
		///<returns>FOUNDERPCB_FRIGHTE_01对象</returns>		
		public FOUNDERPCB_FRIGHTE_01 getFOUNDERPCB_FRIGHTE_01ByRKEY(decimal rkey)
		{
			...
			string sql=@"select top 1 
				...
				,
				isNull(up_module_id,'') as up_module_id
				,
				isNull(module_id,'') as module_id
				,
				isNull(module_english,'') as module_english
				,
				isNull(module_name,'') as module_name
				,
				isNull(module_desc,'') as module_desc
				
			from FOUNDERPCB_FRIGHTE_01 with (nolock) where RKEY='{0}'";
             
			#endregion
			
			///定义返回对象
			FOUNDERPCB_FRIGHTE_01  founderpcb_frighte_01=null;
			
			#region 数据库操作
            try
            {
				 founderpcb_frighte_01=new FOUNDERPCB_FRIGHTE_01();
				
				using(DataTable tb=dbHelper.GetDataSet(string.Format(sql,rkey)) )
				{
					foreach(DataRow row in tb.Rows)
					{	
							    founderpcb_frighte_01.RKEY=decimal.Parse(row["RKEY"].ToString()) ;
								   founderpcb_frighte_01.UP_MODULE_ID =row["UP_MODULE_ID"].ToString();
								   founderpcb_frighte_01.MODULE_ID =row["MODULE_ID"].ToString();
								   founderpcb_frighte_01.MODULE_ENGLISH =row["MODULE_ENGLISH"].ToString();
								   founderpcb_frighte_01.MODULE_NAME =row["MODULE_NAME"].ToString();
								   founderpcb_frighte_01.MODULE_DESC =row["MODULE_DESC"].ToString();
		...
		///<returns>IList<FOUNDERPCB_FRIGHTE_01>数据集合</returns>		
		public IList< FOUNDERPCB_FRIGHTE_01> FindBySql(string sqlWhere)
		{
			...
			from FOUNDERPCB_FRIGHTE_01 with (nolock)";
			...
			IList<FOUNDERPCB_FRIGHTE_01> resultList=new List<FOUNDERPCB_FRIGHTE_01>();
            ...
            { 
				using(DataTable tb=dbHelper.GetDataSet(sql)) 
				{
					foreach(DataRow row in tb.Rows)
					{
							FOUNDERPCB_FRIGHTE_01  founderpcb_frighte_01 =new FOUNDERPCB_FRIGHTE_01();
							...
							resultList.Add(founderpcb_frighte_01);
					}
				}
            }
            catch (Exception e)
