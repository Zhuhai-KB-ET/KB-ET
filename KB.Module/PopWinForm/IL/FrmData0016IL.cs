...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0016IL
    {
        #region 实例化
        DL.FrmData0016DL FrmData0016DL = null;
        FrmData0016 Frm;
        public FrmData0016IL(FrmData0016 psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0016_Load(object sender, EventArgs e)
        {
            try
            {
                FrmData0016DL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0016DL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                FrmData0016DL.BinData(" LOCATION like '%" + Frm.textBoxFind.Text.Trim() + "%' or CODE like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    FrmData0016DL.BinData("CODE like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
