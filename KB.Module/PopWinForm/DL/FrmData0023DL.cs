...
namespace FOUNDERPCB.Module.PopWinForm.DL
{
    ...
    /// 数据层
    /// </summary>
    public partial class FrmData0023DL
    {
        #region 实例化
        FrmData0023 Frm;
        public FrmData0023DL(FrmData0023 psFrm)
        {
            Frm = psFrm; 
        }
        #endregion 
        ...
        public void BinData(string strWhere)
        {
            try
            { 
                DATA0023BLL bll = new DATA0023BLL(Frm.DBConnection);
                IList<DATA0023> IL_DATA0023;
                ...
                tb_GridView1.Columns.Add("供应商代码");
                tb_GridView1.Columns.Add("供应商名称");

                IL_DATA0023 = bll.FindBySql(strWhere);

                for (int i = 0; i < IL_DATA0023.Count; i++)
                {
                    dr = tb_GridView1.NewRow();
                    dr[0] = IL_DATA0023[i].RKEY.ToString();
                    dr[2] = IL_DATA0023[i].CODE.Trim();
                    dr[3] = IL_DATA0023[i].SUPPLIER_NAME.Trim();
                    tb_GridView1.Rows.Add(dr);
                }

                tb_GridView1.DefaultView.Sort = "供应商代码 ASC";
