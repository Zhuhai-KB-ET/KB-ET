...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmData0010IL
    {
        #region 实例化
        DL.FrmData0010DL frmData0010DL = null;
        FrmData0010 Frm;
        public FrmData0010IL(FrmData0010 psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmData0010_Load(object sender, EventArgs e)
        {
            try
            {
                frmData0010DL = new FOUNDERPCB.Module.PopWinForm.DL.FrmData0010DL(Frm);
        ...
        public void textBoxFind_TextChanged(object sender, EventArgs e)
        {
            try
            {
                frmData0010DL.BinData(" CUSTOMER_NAME like '%" + Frm.textBoxFind.Text.Trim() + "%' or CUST_CODE like '%" + Frm.textBoxFind.Text.Trim() + "%'");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
        ...
        public void dataGridView1_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Back)
                {
                    ...
                    frmData0010DL.BinData("CUST_CODE like '%" + Frm.statusStrip1.Items[0].Text.Trim() + "%'");
                }
                else
