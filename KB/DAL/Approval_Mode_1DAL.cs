...
{
	/// <summary>
	/// 数据访问类Approval_Mode_1DAL。
	/// </summary>
	public partial class Approval_Mode_1DAL
	{
		...
		#region 构造方法
		public Approval_Mode_1DAL(DBHelper DB)
		{
				this.FactoryID =  DB.FactoryID;
				this.dbHelper = DB;
		}
		public Approval_Mode_1DAL(int factoryID)
		{
		...
		public int GetMaxId()
		{
		return dbHelper.GetMaxID("RKEY", "Approval_Mode_1"); 
		}
		...
		public bool Exists(int RKEY)
		{
			...
			strSql.Append("select count(1) from Approval_Mode_1 with(nolock) ");
		...
		public void Delete(int RKEY)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Approval_Mode_1 ");
			strSql.Append(" where RKEY=@RKEY ");
			SqlParameter[] parameters = {
					new SqlParameter("@RKEY", SqlDbType.Int,4)};
			parameters[0].Value = RKEY;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select RKEY,FILE_POINTER,SOURCE_TYPE,FROM_STEP_NO,TO_STEP_NO,TRANS_TYPE,TRANS_DESCRIPTION,USER_PTR,TRANS_DATE_TIME ");
			strSql.Append(" FROM Approval_Mode_1 with(nolock)  ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" RKEY,FILE_POINTER,SOURCE_TYPE,FROM_STEP_NO,TO_STEP_NO,TRANS_TYPE,TRANS_DESCRIPTION,USER_PTR,TRANS_DATE_TIME ");
			strSql.Append(" FROM Approval_Mode_1 with(nolock) ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "Approval_Mode_1";
