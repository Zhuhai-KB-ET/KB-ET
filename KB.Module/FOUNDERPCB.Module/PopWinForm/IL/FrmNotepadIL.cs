...
namespace FOUNDERPCB.Module.PopWinForm.IL
{
    ...
    /// 业务中间层
    /// </summary>
    public partial class FrmNotepadIL
    {
        #region 实例化
        DL.FrmNotepadDL frmNotepadDL = null;
        FrmNotepad Frm;
        public FrmNotepadIL(FrmNotepad psFrm)
        {
            Frm = psFrm;
        ...
        public void FrmNotepad_Load(object sender, EventArgs e)
        {
            try
            {
                frmNotepadDL = new FOUNDERPCB.Module.PopWinForm.DL.FrmNotepadDL(Frm);

                frmNotepadDL.BinData("1=1");
            }
            catch (Exception e1)
            {
                log.PrintInfo(e1);
